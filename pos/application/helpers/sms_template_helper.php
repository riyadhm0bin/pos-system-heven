			...
			if(!empty($content)){
				switch ($template_id) {
					case 1:
						...
						$CI->db->select("a.store_id,a.customer_id,b.customer_name,b.mobile,a.sales_code,
												   a.sales_date,a.grand_total,a.paid_amount");
						$CI->db->from("db_sales a");
						$CI->db->where("a.id",$data_id);
						$CI->db->join("db_customers b","b.id=a.customer_id");
					...
					case 2:
						/* SALES RETURN SMS */
						$q2=$CI->db->query("SELECT a.store_id,a.customer_id,b.customer_name,b.mobile,a.return_code,
...
if ( ! function_exists('is_sms_enabled'))
{
    function is_sms_enabled(){
    	$CI =& get_instance();
    	$sms_status=$CI->db->select('sms_status')
    			->where("id",get_current_store_id())
    			->get('db_store')->row()->sms_status;
