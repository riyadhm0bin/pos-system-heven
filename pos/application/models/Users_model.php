...
class Users_model extends CI_Model {
	...
	public function verify_and_save($data){
		...
		$query=$this->db->query("select username from db_users where upper(username)='".strtoupper($username)."'")->num_rows();
	...
	public function verify_and_update($data){
		...
		$query=$this->db->query("select username from db_users where upper(username)='".strtoupper($username)."' and id<>$q_id")->num_rows();
		if($query>0){ return "This username already exist.";}
