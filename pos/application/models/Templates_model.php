...
class Templates_model extends CI_Model {
	...
	public function verify_and_save(){
		...
		if($query->num_rows()>0){
			...
			$query1="insert into db_smstemplates(template_name,content,status,store_id) 
	...
	public function update_template(){
		...
		$store_id = get_current_store_id();
		//Validate This category already exist or not
		$query=$this->db->query("select * from db_smstemplates where upper(template_name)=upper('$template_name') and id<>$q_id and store_id=".$store_id);
		if($query->num_rows()>0){
			return "This Category Name already Exist.";
			
		}
		else{
			$query1="update db_smstemplates set template_name='$template_name',content='$content' where id=$q_id and store_id=".$store_id;
			if ($this->db->simple_query($query1)){
