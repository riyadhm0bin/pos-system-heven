...
$('#save,#update').on("click",function (e) {
    ...
    if(this_id=="save")  //Save start
    {
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"variants/view";
								return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					//if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'variants/update_variant',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
{
	var base_url=$("#base_url").val();
	$.post(base_url+"variants/update_status",{id:id,status:status},function(result){
		if(result=="success")
				{
...
//Delete Record start
function delete_variant(q_id)
{
	var base_url=$("#base_url").val();
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post(base_url+"variants/delete_variant",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
...
function multi_delete(){
    ...
    var this_id=this.id;
		...
		if(confirm("Are you sure ?")){
			...
			$.ajax({
			type: 'POST',
			url: base_url+'variants/multi_delete',
			data: data,
			cache: false,
			contentType: false,
			processData: false,
